...
==========================================================
Script Purpose:
	This stored procedure transforms the ETL (extract, transform, Load) process to
	populate the 'silver' schema tables from the 'bronze' schema.
	It performs the following actions:
	- Truncates the Silver tables before loading the data.
	- Inserts transformed and cleansed data friom Bronze into Silver tables.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT
			cst_id,
			cst_key,
			TRIM(cst_firstname) AS cst_firstname, -- removes unwanted spacing
			TRIM(cst_lastname) AS cst_lastname,
			CASE
				WHEN UPPER(TRIM(cst_marital_status)) = 'S' THEN 'Single'
				WHEN UPPER(TRIM(cst_marital_status)) = 'M' THEN 'Married'
				ELSE 'n/a'
			END cst_marital_status, -- normalise marital status to redable format
			CASE
				WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				ELSE 'n/a' -- replaces NULL with n/a
			END cst_gndr, -- normalise gender to redable format
			cst_create_date
		FROM 
			(SELECT *,
		...
		SELECT
			...
			CASE 
				...
				ELSE TRIM(cntry)
			END AS cntry -- Normalise and Handle missing or blank country code
